...
export function journeyDelayMsFromConfig(config: Record<string, unknown>): number {
  return journeyDelayPartsToMs(normalizeJourneyDelayConfig(config));
}
...
export function isJourneyDelayConfigured(config: Record<string, unknown>): boolean {
  return journeyDelayMsFromConfig(config) > 0;
}
