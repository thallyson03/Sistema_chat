...
router.get('/go', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user?.id) {
      return res.status(401).json({ error: 'Não autenticado' });
    }
    ...
    const msg = e instanceof Error ? e.message : 'Erro ao redirecionar para portal';
    console.error('[ticketPortalRoutes/go]', msg);
    return res.status(500).json({ error: msg });
  }
});
